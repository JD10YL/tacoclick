...
function buy_computer_menu () {
    ...
    if (score2 >= computer_price || debug) {
...
function passed_requirements () {
    ...
    if (score2 < blockObject.getNumberProperty(local_requirements, NumProp.score)) {
        return false
    }
...
function load_progress () {
    ...
    score2 = blockSettings.readNumber("game_score")
...
function buy_asic_menu () {
    ...
    if (blockMenu.selectedMenuOption().includes("Cancel")) {
        ...
        asic_count += -1
        score2 += Math.round(asic_price * 0.8)
...
function buy_autoclicker_menu () {
    ...
    if (score2 >= autoclicker_price || debug) {
    ...
    if (blockMenu.selectedMenuOption().includes("Cancel")) {
        ...
        autoclicker_count += -1
        score2 += Math.round(autoclicker_price * 0.8)
        autoclicker_price = autoclicker_price / 1.1
    } else if (blockMenu.selectedMenuOption().includes("Buy")) {
        autoclicker_count += 1
        score2 += autoclicker_price * -1
        autoclicker_price = autoclicker_price * 1.1
    }
...
function wipe_save () {
    for (let value2 of blockSettings.list()) {
        if (value2.indexOf("game") == 0) {
            blockSettings.remove(value2)
        }
    }
    set_default_save()
}
spriteutils.createRenderable(0, function (screen2) {
    ...
    images.print(screen2, "MakeCoins: " + score2, 2, 2, 1)
...
spriteutils.createRenderable(0, function (screen2) {
    ...
    for (let index = 0; index <= 4; index++) {
        images.print(screen2, "TACOTACO", 1, 82 + index * 7, 1)
    }
})
function overlapped_sprites (sprite: Sprite, kind: number) {
    local_overlapping_sprites = []
    for (let value3 of sprites.allOfKind(kind)) {
        if (sprite.overlapsWith(value3)) {
            local_overlapping_sprites.push(value3)
        }
    }
...
function save_progress () {
    ...
    blockSettings.writeNumber("game_score", score2)
...
function get_upgrades_menu () {
    ...
    for (let value4 of all_upgrades) {
        if (upgrades_obtained.indexOf(blockObject.getNumberProperty(value4, NumProp.id)) != -1) {
            continue;
        }
        local_requirements = blockObject.getAnyProperty(value4, AnyProp.requirements)
        if (passed_requirements()) {
            local_available_upgrades.push(value4)
            local_menu_options.push("" + blockObject.getStringProperty(value4, StrProp.name) + ": ($" + blockObject.getNumberProperty(value4, NumProp.cost) + ") " + blockObject.getStringProperty(value4, StrProp.description))
    ...
    if (blockMenu.selectedMenuOption().includes("Cancel")) {
        ...
        if (score2 >= blockObject.getNumberProperty(local_upgrade_got, NumProp.cost) || debug) {
            score2 += blockObject.getNumberProperty(local_upgrade_got, NumProp.cost) * -1
...
game.onUpdate(function () {
    if (controller.A.isPressed()) {
        ...
        for (let value5 of overlapped_sprites(sprite_cursor_pointer, SpriteKind.Shop)) {
            value5.image.replace(9, 8)
        }
    ...
    for (let value6 of sprites.allOfKind(SpriteKind.Shop)) {
        if (sprite_cursor_pointer.overlapsWith(value6)) {
            value6.image.replace(1, 9)
        } else {
            value6.image.replace(8, 9)
            value6.image.replace(9, 1)
        }
    }
})
...
forever(function () {
    timer.throttle("tick", 1000 / max_ticks_per_second, function () {
        for (let index2 = 0; index2 <= autoclicker_count - 1; index2++) {
            timer.throttle("autoclicker_click_" + index2, autoclicker_speed, function () {
                computer_click()
            })
        }
        for (let index3 = 0; index3 <= computer_count - 1; index3++) {
            timer.throttle("computer_mine_" + index3, computer_speed, function () {
                check_for_magic_number(randint(0, max_height))
                hash_count_per_sec += 1
            })
        }
        for (let index4 = 0; index4 <= asic_count - 1; index4++) {
            timer.throttle("asic_mine_" + index4, asic_speed, function () {
                check_for_magic_number(randint(0, max_height))
                hash_count_per_sec += 1
            })
        }
